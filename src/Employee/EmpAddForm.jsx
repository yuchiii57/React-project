...
export default () => {
  const [empId, setEmpId] = useState("");  //設定欄位
  ...
  const [residenceAddr, setResidenceAddr] = useState("");
  const [contactAddr, setContactAddr] = useState("");
  const [contactTel, setContactTel] = useState("");
  const [cellphone, setCellphone] = useState("");
  const [contact01, setContact01] = useState("");
  const [contactTel01, setContactTel01] = useState("");
  const [contact02, setContact02] = useState("");
  const [contactTel02, setContactTel02] = useState("");
  const [privateEmail, setPrivateEmail] = useState("");
  const [officialEmail, setOfficialEmail] = useState("");
  const [license01, setLicense01] = useState("");
  const [license02, setLicense02] = useState("");
  const [license03, setLicense03] = useState("");
  const [lineManager, setLineManager] = useState("");
  const [depManager, setDepManager] = useState("");
  const [deputy, setDeputy] = useState("");
  const [resignDT, setResignDT] = useState(moment().format("YYYY-MM-DD"));
  const [memo, setMemo] = useState("");

  const [onCreateEmployeeM] = useMutation(createEmployeeM, {
    variables: {
      empId,
      empNm,
      dep,
      ...
      <Form onSubmit={handleSubmit}>
        <Form.Group>
          <Form.Field
            ...
            control={Input}
            pattern="^\d{9}$"
            placeholder="輸入格式: 109010101 民國年月日+2位序號"
            title="輸入格式: 109010101 民國年月日+2位序號"
            inline
            onChange={e => {
